...
client.hset('beacon_config', 'c30b6529c756a18a', JSON.stringify({beacon: 'c30b6529c756a18a', created:  1529211600000, updated: 1529211600000, alarm: false, alarm_time: 1529287200000, alarm_reason: false, alarms_triggered: 0, muted: false, threshold: 60000, in_ok_zones: [], in_notok_zones: [], zones: ['30aea4500ec4']}));
client.hset('beacon_config', 'c6091790a9118b17', JSON.stringify({beacon: 'c6091790a9118b17', created:  1529211600000, updated: 1529211600000, alarm: false, alarm_time: 1529287200000, alarm_reason: false, alarms_triggered: 0, muted: false, threshold: 60000, in_ok_zones: [], in_notok_zones: [], zones: ['30aea4500ec4']}));

//c30b6529c756a18a: '{"beacon":"c30b6529c756a18a","created":1529211600000,"updated":1529211600000,"alarm":false,"alarm_time":1529287200000,"alarm_reason":false,"alarms_triggered":0,"muted":false,"threshold":60000,"in_ok_zones":[],"in_notok_zones":[],"zones":["30aea4500ec4"]}' }
...
//update functions
function compareBeacon(beacon, beacon_config, beacon_status){
	...
	for(let n in beacon_s_arr){
		...
		if(b_status.beacon == beacon){
			if((time - b_status.updated) < min_update){
						...
						for(let i in beacon_c_obj){
							...
							if(beacon.in_ok_zones.concat(beacon.in_notok_zones).length != 0){
								if(beacon.in_ok_zones.length > 0){
									...
									if((time - beacon_obj.alarm_time) > beacon_obj.threshold){
										...
										if(!beacon_obj.muted) beacon_obj.alarm_time = time, beacon_obj.alarms_triggered ++;
									}
									else{
								...
								if((time - beacon_obj.alarm_time) > beacon_obj.threshold){
									console.log(beacon_obj.beacon +  ' Invisible > threshold ' + (time - beacon_obj.alarm_time));
									beacon_obj.updated = time;
									beacon_obj.alarm = true;
